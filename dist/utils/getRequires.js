'use strict'; // Need to supply the regex test as a string for reuse in unit tests
...
module.exports = function getRequires(code) {
  ...
  code.replace(REQUIRE_ANYTHING_REGEX, function (requireExprMatch, requiredExpr) {
    ...
    if (!requireStrMatch) {
      throw new Error("Requires using expressions are not supported in examples. (Used: ".concat(requireExprMatch, ")"));
    }
